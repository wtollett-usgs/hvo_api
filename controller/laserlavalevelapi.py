...
from model.laserlavalevel import LaserLavaLevel, LaserLavaLevelTranslation
...
class LaserLavaLevelAPI(Resource):
    ...
    def get(self):
        ...
        w_items   = [LaserLavaLevel.timestamp.between(date_to_j2k(start, tz), date_to_j2k(end, tz))]
        ...
        for d in data:
            if d.sealevel is None:
                l({'date': jtod(d.timestamp, tz).strftime('%Y-%m-%d %H:%M:%S.%f'), 
                'rank': d.rank.name, 
                'sealevel': d.sealevel,
                'overlook': d.overlook})
            else:
                l({'date': jtod(d.timestamp, tz).strftime('%Y-%m-%d %H:%M:%S.%f'), 
                'rank': d.rank.name, 
                'sealevel': d.sealevel * d.translation.csealevel + d.translation.dsealevel,
                'overlook': d.overlook * d.translation.coverlook + d.translation.doverlook})
        return { 'nr': len(data),
                 'records': output }, 200
