...
import model.so2highres as so2highres

_series_options = ['so2', 'ws', 'wd', 'wdstd', 'gust', 'xtemp', 'rh' ,'bp', 'tip'
                   'itemp', 'bv', 'flow', 'elecbv']
...
class SO2HighResAPI(Resource):
    def __init__(self):
        ...
        self.reqparse.add_argument('series', type = str, required = False, default = 'so2')
    ...
    def get(self):
        ...
        unknown  = set([x.upper() for x in channels]).difference(so2highres._tablenames)
        if len(unknown) > 0:
