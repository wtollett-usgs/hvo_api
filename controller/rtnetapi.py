...
class RTNetAPI(Resource):
    def __init__(self):
        ...
        self.reqparse.add_argument('series', type = str, required = False, default = 'north,east,up')
    ...
    def get(self):
        ...
        unknown  = set([x.upper() for x in channels]).difference(rtnet._tablenames)
        if len(unknown) > 0:
        ...
        for channel in channels:
            ...
            for i in series:
                if args['debias'] == 'mean':
                    means[i] = sum([getattr(x,i) for x in data]) / float(len(data)) if i in series else 0
                else:
                    means[i] = 0
