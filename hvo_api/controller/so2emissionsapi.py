...
import model.so2emissions as so2emissions

_series_options = ['so2', 'so2sd', 'windspeed', 'winddir', 'samples',
                   'sampleNumber']


class SO2EmissionsAPI(Resource):
    def __init__(self):
        ...
        self.reqparse.add_argument('timezone', type=str, required=False,
                                   default='hst')
        super(SO2EmissionsAPI, self).__init__()
    ...
    def create_param_string():
        ...
        ranks = [{'rid': x.rid, 'name': x.name}
                 for x in so2emissions.SO2EmissionsRank.query.all()]
        params = {}
        params['channel'] = {'type': 'string', 'required': 'yes',
                             'note': 'Can be comma-separated list.',
                             'options': so2emissions._tablenames}
