# -*- coding: utf-8 -*-
from .common_constants import MAX_LINES, DS_OPTIONS, FFMT
...
class GPSAPI(Resource):
    ...
    def get_gps_data(channel, args, dates):
        ...
        id = (GPSChannel
              .query.filter(GPSChannel.code == channel.upper()).one().cid)

        tz = dates[2]
        queryclauses.append(GPSChannel.cid == id)
        queryclauses.append(GPSSource.avgdate
                            .between(date_to_j2k(dates[0], tz),
        ...
        if args['downsample'] == 'none':
            ...
            q_items = []
            interval = args['dsint']
            q_items.extend([func.min(GPSSource.avgdate).label('t'),
                            func.min(GPSRank.rid).label('r')])
            q_items.extend([func.avg(Solution.x).label('x'),
            ...
            q = (db.session.query(*q_items)
                   .select_from(Solution).join(GPSChannel, GPSSource, GPSRank))
    ...
    @staticmethod
    def apply_baseline(base, d):
        """Subtracts position and adds covariance of specified baseline data
        at subset of times common to both baseline data and data being altered.

        Parameters:
        base -- GPS data from the baseline channel
        ...
        while True:
            ...
            if fabs(st - bt) < 0.001:
                t = st
                r = d['r'][si]
                x = d['xyz'][si*3] - base['xyz'][bi*3]
                y = d['xyz'][si*3+1] - base['xyz'][bi*3+1]
                z = d['xyz'][si*3+2] - base['xyz'][bi*3+2]
                sxx = d['cov'][si*3, si*3] + base['cov'][bi*3, bi*3]
                syy = d['cov'][si*3+1, si*3+1] + base['cov'][bi*3+1, bi*3+1]
                szz = d['cov'][si*3+2, si*3+2] + base['cov'][bi*3+2, bi*3+2]
                sxy = d['cov'][si*3, si*3+1] + base['cov'][bi*3, bi*3+1]
                sxz = d['cov'][si*3, si*3+2] + base['cov'][bi*3, bi*3+2]
                syz = d['cov'][si*3+1, si*3+2] + base['cov'][bi*3+1, bi*3+2]
                nlen = sqrt(x*x + y*y + z*z)
                newdata.append(GPSDataPoint(t, r, x, y, z, sxx, syy, szz,
                                            sxy, sxz, syz, nlen))
                si += 1
                bi += 1
            else:
                if st < bt:
                    si += 1
                    while si < len(d['t']) and d['t'][si] < base['t'][bi]:
                        si += 1
                    if si == len(d['t']):
                        break
                else:
                    bi += 1
                    while bi < len(base['t']) and base['t'][bi] < d['t'][si]:
                        bi += 1
                    if bi == len(base['t']):
                        break
            if si == len(d['t']) or bi == len(base['t']):
                break
        return GPSAPI.set_to_list(newdata)

    @staticmethod
    def set_to_list(d):
        """Given raw data, create data structures for use in
        later calculations"""
        if len(d) == 0:
            return {}
        else:
    ...
    def xyz_to_llh(x, y, z):
        ...
        theta = atan((z * a) / (p * b))
        llh.append(atan((z + E2 * b * fpow(sin(theta), 3))
    ...
    def to_enu(lon, lat, size, xyz, cov):
        ...
        for i in range(size):
            ...
            tmpxyz.append(t * matrix([[xyz[i*3]],
                                      [xyz[i * 3 + 1]],
                                      [xyz[i * 3 + 2]]]))
            copyto(tmpcov[i*3:i*3+3, i*3:i*3+3],
                   (t * cov[i*3:i*3+3, i*3:i*3+3]) * tt)

        return concatenate(tmpxyz), tmpcov

    @staticmethod
    def createENUTransform(lon, lat):
        """Creates a transformation matrix for tranlating from
        lon/lat/height to east/north/up"""
    ...
    def create_param_string():
        ...
        params['baseline'] = {'type': 'string', 'required': 'no',
                              'note': 'Can be comma-separated list.',
                              'options': gps_channels}
