...
class RSAMAPI(Resource):
    ...
    def get(self):
        ...
        for channel in channels:
            ...
            if args['downsample'] == 'none':
                ...
                data = (db.session
                          .execute(text(s),
                                   params=dict(st=date_to_j2k(start, tz),
                ...
                q_items.append(((cname.timestamp)-date_to_j2k(start, tz))
                               .self_group().op('div')(interval)
                               .label('intNum'))
                q = (db.session.query(*q_items).filter(*queryclauses)
    ...
    def create_param_string():
        ...
        params['starttime'] = {'type': 'string', 'required': 'yes',
                               'note': ('Will also accept things like '
                                        '-6m for last six months.'),
