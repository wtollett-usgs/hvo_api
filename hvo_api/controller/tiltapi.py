...
_series_options = ['all', 'radial', 'tangential', 'east', 'north',
                   'magnitude', 'azimuth', 'holeTemp', 'boxTemp', 'instVolt',
                   'rainfall']
...
class TiltAPI(Resource):
    def __init__(self):
        ...
        self.reqparse.add_argument('timezone', type=str, required=False,
                                   default='hst')
        self.reqparse.add_argument('azimuth', type=str, required=False,
                                   default='nominal')
    ...
    def get(self):
        ...
        for channel in channels:
            ...
            if args['downsample'] == 'none':
                ...
                q_items = []
                q_items.extend([cname.timestamp.label('timestamp'),
                                cname.rid.label('rid')])
                q_items.append((func.cos(func.radians(tt.azimuth))
                                * (cname.xTilt * tt.cxTilt
                                + tt.dxTilt).self_group()
                                + func.sin(func.radians(tt.azimuth))
                                * (cname.yTilt * tt.cyTilt
                                + tt.dyTilt).self_group()).label('east'))
                q_items.append((-func.sin(func.radians(tt.azimuth))
                                * (cname.xTilt * tt.cxTilt
                                + tt.dxTilt).self_group()
                                + func.cos(func.radians(tt.azimuth))
                                * (cname.yTilt * tt.cyTilt
                                + tt.dyTilt).self_group()).label('north'))
                q_items.extend([tilt.TiltRank.name, tt])

                # Add optional parameters
                if any(x in args['series'] for x in ['holeTemp', 'all']):
                    q_items.append((cname.holeTemp
                                    * tt.choleTemp
                                    + tt.dholeTemp).label('holeTemp'))
                if any(x in args['series'] for x in ['boxTemp', 'all']):
                    q_items.append((cname.boxTemp
                                    * tt.cboxTemp
                                    + tt.dboxTemp).label('boxTemp'))
                if any(x in args['series'] for x in ['instVolt', 'all']):
                    q_items.append((cname.instVolt
                                    * tt.cinstVolt
                                    + tt.dinstVolt).label('instVolt'))
                if any(x in args['series'] for x in ['rainfall', 'all']):
                    q_items.append((cname.rainfall
                                    * tt.crainfall
                                    + tt.drainfall).label('rainfall'))

                q = db.session.query(*q_items).join(tt,
                                                    tilt.TiltRank)
        ...
        if set(args['series'].split(',')).intersection(['all', 'radial',
                                                        'tangential',
                                                        'magnitude', 'azimuth',
                                                        'east', 'north']):
            tc = tilt.TiltChannel
    ...
    def create_param_string():
        ...
        ranks = [{'rid': x.rid, 'name': x.name}
                 for x in tilt.TiltRank.query.all()]
        params = {}
        ...
        params['azimuth'] = {'type': 'string', 'required': 'no',
                             'default': 'nominal', 'options': _azimuth_options}
