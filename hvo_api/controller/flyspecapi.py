...
from model import flyspec
from valverest.util import create_date_from_input, date_to_j2k, j2k_to_date

_series_options = ['bstflux', 'bstfluxmean', 'bstfluxmeanstdev', 'flux01',
                   'fluxsdh', 'ps', 'pd', 'specs', 'nflag', 'sflag', 'colpath',
                   'satpix', 'inttime', 'boxtemp', 'sysvolt', 'current']
...
class FlyspecAPI(Resource):
    ...
    def get(self):
        ...
        unknown = (set([x.upper() for x in channels])
                   .difference(flyspec._tablenames))
        if len(unknown) > 0:
        ...
        for channel in channels:
            ...
            for d in data:
                a = {'date': Date(d.timestamp, tz).strftime(FFMT),
                     'rank': d.rank.name}
    ...
    def create_param_string():
        ...
        ranks = [{'rid': x.rid, 'name': x.name}
                 for x in flyspec.FlyspecRank.query.all()]
        params = {}
